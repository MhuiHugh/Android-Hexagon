package com.hui.core.imageloader;
...
import com.hui.core.imageloader.listener.IGetBitmapListener;
import com.hui.core.imageloader.listener.IGetDrawableListener;
import com.hui.core.imageloader.listener.IImageLoaderListener;
import com.hui.core.imageloader.listener.ImageSize;
import com.hui.core.imageloader.okhttp.OnGlideImageViewListener;
import com.hui.core.imageloader.okhttp.OnProgressListener;
import com.hui.core.imageloader.okhttp.ProgressManager;
import com.hui.core.imageloader.tranform.BlurBitmapTranformation;
import com.hui.core.imageloader.tranform.GlideCircleTransformation;
import com.hui.core.imageloader.tranform.RoundBitmapTranformation;
 ...
 * form https://github.com/Shimingli/ImageLoader
 */
...
public class GlideImageLoaderClient implements IImageLoaderClient {
     ...
     * @param imageView
     * @param holderRes        占位图
     * @param errorRes      加载失败图
     * @param listener
     * @param cacheInMemory
     */
    @Override
    public void displayImage(Context context, String url, ImageView imageView, int holderRes, int errorRes, IImageLoaderListener listener, boolean cacheInMemory) {
        GlideApp.with(context).load(url).diskCacheStrategy(DiskCacheStrategy.AUTOMATIC)
                .placeholder(holderRes).error(errorRes).skipMemoryCache(cacheInMemory)
     ...
     */
    @Override
    public void displayImage(Context context, String url, ImageView imageView, Drawable holderRes, Drawable errorRes, IImageLoaderListener listener, boolean cacheInMemory) {
        GlideApp.with(context).load(url).diskCacheStrategy(DiskCacheStrategy.AUTOMATIC)
                .placeholder(holderRes).error(errorRes).skipMemoryCache(cacheInMemory)
                .listener(new RequestListener<Drawable>() {
                    @Override
...
//                .into(imageView);
    ...
    //图片下载
    @Override
    public void downLoadImage(Context context, String url, IImageLoaderListener listener) {
        GlideApp.with(context).load(url).skipMemoryCache(true).diskCacheStrategy(DiskCacheStrategy.AUTOMATIC).listener(new RequestListener<Drawable>() {
            @Override
            public boolean onLoadFailed(@Nullable GlideException e, Object model, Target<Drawable> target, boolean isFirstResource) {
                if(null!=listener) {
                    listener.onLoadingFailed(url, null, e);
                }
                return false;
            }
            ...
            public boolean onResourceReady(Drawable resource, Object model, Target<Drawable> target, DataSource dataSource, boolean isFirstResource) {
                if(null!=listener) {
                    listener.onLoadingComplete(url, null);
                }
                return false;
            }
        }).submit();
     ...
     */
    @Override
    public boolean hasImageCacheUrl(Context context, String url) {
        try {
            File file=Glide.with(context).downloadOnly().skipMemoryCache(true).load(url).apply(new RequestOptions().onlyRetrieveFromCache(true)).submit().get();
            if(null!=file){
                return true;
            }else {
                return false;
    ...
    /**
     * 判断是否有缓存，同步方法,结果返回大概20毫秒
     * @param context
     * @param url
     * @return
     */
    @Override
    public boolean hasImageCacheUrlSync(Context context, String url) {
        try {
            File file=getCache(context,url);
            if(null!=file){
                return true;
            }else {
                return false;
    ...
    /**
     * 反射方式获取Glide缓存,结果返回大概20毫秒
     * @param context
     * @param url
     * @return
     * https://blog.csdn.net/yi18710630585/article/details/106708863
     */
    private File getCache(Context context,String url){
        File cacheFile = null;
